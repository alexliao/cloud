...
def authorize
  ...
  redirect_to :action => "login" unless (session[:is_admin] || (current_user and current_user.is_admin))
end
...
def login
  ...
  if request.get?
    ...
    if params[:admin][:password] == ENV['admin_pwd']
      ...
      flash[:notice] = "<span class='notice'>密码错误，请重新输入!</span><br/><br/>"
    end
  end
end

def invite
  @users_count_all = User.count
