...
class Photo 
  ...
  def mask
    Photo.ensure_directory(path) + Photo.gen_windows_filename(path) + '_[size].jpg'
  end
  ...
  def self.thumbnail(picture_url, size)
    thumbnail_url = ensure_directory(picture_url) + thumbnail_name(picture_url, size) + '.jpg'
